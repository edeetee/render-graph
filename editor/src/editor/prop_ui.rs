...
use graph::{
    ...
    def::{RangedData, Reset, TextStyle},
};
use serde::{Deserialize, Serialize};

use crate::{
    editor::animation_ui::draw_dataupdater,
    widgets::limited_ui::{horizontal_drags, UiLimit},
};
...
fn draw_param(param: &mut UiValue, ui: &mut Ui, param_name: &str) -> ParamUiResponse {
    ...
    match &mut param.0 {
        ...
        Menu(RangedData { value, .. }, ref label_mapping) => ui
        ...
        Text(RangedData { value, .. }, style) => ui
            .horizontal(|ui| {
                ui.label(param_name);
                        ...
                        match animator {
                            ...
                            None => {
                                if ui.button("ANIMATE").clicked() {
                                    if let Some(updater) = DataUpdater::from_param(&self.0) {
                                        user_state
                                            .animator
                                            .animations
